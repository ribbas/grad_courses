...
void parseEquation(char*& ch, SS_Cell* cell) {
	...
	if (left->getKind() == ID) cell->setLeft(left->getLexeme());
	else cout << "Error: unrecognized left ID token" << endl << flush;
	delete left;

	Token* op = getToken(ch);
	int kind = op->getKind();
	if (kind == ADD) cell->setOp(op_ADD);
	else if (kind == SUB) cell->setOp(op_SUB);
	else if (kind == MULT) cell->setOp(op_MULT);
	else cout << "Error: unrecognized operator token" << endl << flush;
	delete op;

	Token* right = getToken(ch);
	if (right->getKind() == ID) cell->setRight(right->getLexeme());
	else cout << "Error: unrecognized right ID token" << endl << flush;
	delete right;
