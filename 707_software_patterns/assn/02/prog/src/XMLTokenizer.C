...
XMLTokenizer::XMLToken *	XMLTokenizer::getNextToken(void)
{
	...
	if (inside_tag)
	{
		...
		if (pending_attribute_value && boost::regex_search(line, what, attribute_value))
		{
...
void		XMLTokenizer::update_matchers(const boost::ssub_match & matcher, const boost::ssub_match & suffix)
{
