...
const char* XMLTokenizer::ATTRIBUTE =
...
XMLTokenizer::XMLTokenizer(const std::string& filename)
    : file(filename.c_str(), std::ios_base::in), line_number(0), index(0),
      inside_tag(false), pending_attribute_value(false), tag_found(false),
...
XMLTokenizer::XMLToken* XMLTokenizer::getNextToken() {
    ...
    if (inside_tag) {
        if (!tag_found && boost::regex_search(line, what, prolog_identifier)) {
            XMLToken* token =
                new XMLToken(std::string(what[0].first, what[0].second),
                             XMLToken::PROLOG_IDENTIFIER);
            update_matchers(what[0], what.suffix());
            tag_found = true;
            return token;
        }

        if (!tag_found && boost::regex_search(line, what, element)) {
            XMLToken* token = new XMLToken(
                std::string(what[0].first, what[0].second), XMLToken::ELEMENT);
            update_matchers(what[0], what.suffix());
            tag_found = true;
            return token;
        }

        if (pending_attribute_value &&
            boost::regex_search(line, what, attribute_value)) {
            XMLToken* token =
                new XMLToken(std::string(what[0].first, what[0].second),
                             XMLToken::ATTRIBUTE_VALUE);
            update_matchers(what[0], what.suffix());
            pending_attribute_value = false;
            return token;
        }

        if (boost::regex_search(line, what, attribute)) {
            XMLToken* token =
                new XMLToken(std::string(what[0].first, what[0].second),
                             XMLToken::ATTRIBUTE);
            update_matchers(what[0], what.suffix());
        ...
        if (boost::regex_search(line, what, null_tag_end)) {
            XMLToken* token =
                new XMLToken(std::string(what[0].first, what[0].second),
                             XMLToken::NULL_TAG_END);
            inside_tag = false;
            pending_attribute_value = false;
            tag_found = false;
            update_matchers(what[0], what.suffix());
        ...
        if (boost::regex_search(line, what, tag_end)) {
            XMLToken* token = new XMLToken(
                std::string(what[0].first, what[0].second), XMLToken::TAG_END);
            inside_tag = false;
            pending_attribute_value = false;
            tag_found = false;
            update_matchers(what[0], what.suffix());
    ...
    if (boost::regex_search(line, what, prolog_start)) {
        XMLToken* token = new XMLToken(
            std::string(what[0].first, what[0].second), XMLToken::PROLOG_START);
        inside_tag = true;
        update_matchers(what[0], what.suffix());
        return token;
    }

    if (boost::regex_search(line, what, prolog_end)) {
        XMLToken* token = new XMLToken(
            std::string(what[0].first, what[0].second), XMLToken::PROLOG_END);
        inside_tag = false;
        update_matchers(what[0], what.suffix());
        return token;
    }

    if (boost::regex_search(line, what, tag_close_start)) {
        XMLToken* token =
            new XMLToken(std::string(what[0].first, what[0].second),
                         XMLToken::TAG_CLOSE_START);
        inside_tag = true;
        update_matchers(what[0], what.suffix());
        return token;
    }

    if (boost::regex_search(line, what, tag_start)) {
        XMLToken* token = new XMLToken(
            std::string(what[0].first, what[0].second), XMLToken::TAG_START);
        inside_tag = true;
        update_matchers(what[0], what.suffix());
        return token;
    }
    if (boost::regex_search(line, what, value)) {
        XMLToken* token = new XMLToken(
            std::string(what[0].first, what[0].second), XMLToken::VALUE);
        update_matchers(what[0], what.suffix());
        return token;
    }

    if (boost::regex_search(line, what, space_to_eol)) {
        update_matchers(what[0], what.suffix());
        return getNextToken();
    }
...
void XMLTokenizer::update_matchers(const boost::ssub_match& matcher,
