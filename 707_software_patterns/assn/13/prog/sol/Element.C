...
void			Element_Impl::setAttribute(const std::string & name, const std::string & value)
{
	...
	dom::Attr *	attribute;
	attributes.push_back(attribute = getOwnerDocument()->createAttribute(name, value));
...
{
	if (getTagName() == "operation")
	{
		if (attributes.empty() || !hasChildNodes())
			return 0;

		dom::Node *	lhs_child	= getFirstChild();
		dom::Node *	rhs_child	= getLastChild();

		if (lhs_child == rhs_child)
		{
			Attr_Impl *	operation	= dynamic_cast<Attr_Impl *>(attributes.getNamedItem("type"));

			if (operation != 0 && operation->getValue() == "-")
				return -dynamic_cast<Element *>(lhs_child)->Interpret();
			else
				return dynamic_cast<Element *>(lhs_child)->Interpret();
		}

		if (dynamic_cast<Element *>(lhs_child) == 0 || dynamic_cast<Element *>(rhs_child) == 0)
			return 0;

		int		lhs		= dynamic_cast<Element *>(lhs_child)->Interpret();
		int		rhs		= dynamic_cast<Element *>(rhs_child)->Interpret();
		Attr_Impl *	operation	= dynamic_cast<Attr_Impl *>(attributes.getNamedItem("type"));
	...
	else if (getTagName() == "value")
	{
		dom::Node *	child	= getFirstChild();

		if (dynamic_cast<Text_Impl *>(child) != 0)
			return atoi(dynamic_cast<Text_Impl *>(child)->getData().c_str());
		else
			return 0;
	}
	else
