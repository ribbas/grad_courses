...
void testTokenizer(int argc, char** argv)
{
	...
	dom::Attr *	attr	= document->createAttribute("NewAttribute", "attribute value");
...
void testSerializer(int argc, char** argv)
{
	...
	dom::Attr *	attr		= document->createAttribute("attribute", "attribute value");
...
void testValidator(int argc, char** argv)
{
	...
	attr		= document->createAttribute("attribute", "attribute value");
	child->setAttributeNode(attr);
	root->appendChild(child);
	child		= new ElementValidator(document->createElement("element"), &xmlValidator);
	root->appendChild(child);
...
void testInterpreter(int argc, char** argv)
{
	...
	XMLValidator	xmlValidator;
	ValidChildren *	schemaElement	= xmlValidator.addSchemaElement("");
	schemaElement->addValidChild("operation", false);
	schemaElement	= xmlValidator.addSchemaElement("operation");
	schemaElement->addValidChild("operation", false);
	schemaElement->addValidChild("value", false);
	schemaElement->addValidChild("type", true);
	schemaElement->setCanHaveText(false);
	schemaElement	= xmlValidator.addSchemaElement("value");
	schemaElement->setCanHaveText(true);

	Director	director(argv[2], &builder);
	std::cout << "Interpretation of input document:  " << document->getDocumentElement()->Interpret() << std::endl;
}
